    ...
    <div>
        ...
        <div class="boxed">
            <div class="boxed-title">
                <div class="boxed-item-center title">Эрүүл мэнд</div>
            </div>
            <v-server-table ref="tableni" :url="url"  v-if="fetched" :columns="columns" :options="options">
                <div slot="col1" slot-scope="props" class="data-action">
                    төрсөн хүний тоо: {{props.row.birth}}<br>
                    нас барсан хүний тоо: {{props.row.die}}<br>
                </div>
                <div slot="col2" slot-scope="props" class="data-action">
                    алсын дуудлага: {{props.row.call_remote}}<br>
                    ойрын дуудлага: {{props.row.call_near}}<br>

                </div>
                <div slot="col3" slot-scope="props" class="data-action">
                    урьдчилан сэргийлэх үзлэг: {{props.row.inspection}}<br>
                    хэвтэн эмчлүүлэгч: {{props.row.inpatient}}<br>
                    ЯТТ: {{props.row.ytt}}
                </div>
                <div slot="action" slot-scope="props" class="data-action">
                    <router-link :to="'dashboard_hospital/'+props.row.id+'/update'" ><i class="fas fa-pencil-alt"></i></router-link>
            ...
            <div class="boxed-item-center absolute">
                <router-link :to="{ name: 'create_dashboard_hospital'}" class="add_button">+</router-link>
            </div>
...
<script>
    export default {
        data(){
            return {
                siteUrl: window.surl,
                url: '/dashboard_hospital',
                deletemodal:false,
                deleteid: false,
                fetched:false,
                is_loading:false,
                user:false,
                columns: ['id', 'hospical_date',   'site_id', 'user_name',  'col1',  'col2',   'col3',  'action'],
                options: {
                    ...
                    sortable: [ 'hospical_date', 'site_id',  ],
                    filterable: ['hospical_date', 'site_id',  ],
                    listColumns: {
                        site_id:[],
                    },
                    texts:{
        ...
        methods: {
            ...
            ustga(row){
                ...
                axios.delete('/dashboard_hospital/'+row.id).then((response) => {
