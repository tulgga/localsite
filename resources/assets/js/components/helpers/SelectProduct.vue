<template>
    <div>
        <input type="text" class="input" v-model="subsText" @click="chooseSubs" :placeholder="$store.getters.lang.form_text.name" readonly="readonly"/>
                        ...
                        <div class="columns is-mobile is-multiline">
                            <div class="column is-12-mobile is-12-tablet">
                                <template v-for="sub in filteredItems">
                                    <div class="field">
                                        <input class="is-checkradio" :id="'sub'+sub.id" type="radio" :value="sub.product_key" v-model="smodel.id">
                                        <label :for="'sub'+sub.id" @click="subsText = sub.name">{{sub.name}}</label>
                                    </div>
                                </template>
                            </div>
                        </div>
    ...
    export default {
        ...
        created: function () {
            if (this.subItems.length > 0) {
                    let indexni = this.getIndexIfObjWithOwnAttr(this.subItems, 'product_key', this.smodel.id);
                    this.subsText = this.subItems[indexni].name; 
                }
            if(!this.chooseModal){
                this.search = '';
            }
