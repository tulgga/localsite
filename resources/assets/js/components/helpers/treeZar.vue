                ...
                <div  class="tree-content">
                    <div  v-if="vmIdx" class="columns ">
                        ...
                        <div class="column is-5">
                            <input placeholder="Нэр" type="text" v-model="data.name"  :name="'title'+data.id"  v-validate="{'required':true, 'min':3}"  :class="{'input': true, 'is-danger': errors.has('title'+data.id) }"/>
                            <p v-show="errors.has('title'+data.id)" class="help is-danger">Заавал бөглө, хамгийн багадаа 3 тэмдэгт байна.</p>
                        </div>
                        <div class="column is-5">
                            <input  placeholder="Зурагны холбоос" type="text" v-model="data.image" :name="'image'+data.id"   :class="{'input': true, }"/>
                        </div>
                    ...
                    <div v-else class="boxed-item-center title" style="font-size: 1.75rem; font-weight: 400; text-transform: uppercase; letter-spacing: 4px;margin-bottom: 0; padding:0.5em">{{data.name}}</div>
                </div>
            </template>
